...
def build_group_matrix(D_arr, sigma_r_arr, h):
    ...
    for i in range(N):
        ...
        # off-diagonal terms
        if i>0:
            D_left      = 2*D_arr[i]*D_arr[i-1]/(D_arr[i]+D_arr[i-1])
            L[i,i-1]    = -D_left / h**2 
        if i< N-1:
            D_right     = 2*D_arr[i]*D_arr[i+1]/(D_arr[i]+D_arr[i+1])
            L[i,i+1]    = -D_right/h**2 
    return L 
...
class DiffusionSolver1D:
    ...
    def build_matrices(self):
        ...
        S12 = np.diag(-mesh.sigma_s12)              # hint: sigma_s12 array
    ...
    def solve(self, max_iter=1000, tol=1e-6):
        ...
        ## ── DEBUG: verify matrix structure ──────────────────────
        #print("\n--- Matrix Verification ---")
        #print(f"L matrix shape: {self.L.shape}")
        #print(f"F matrix shape: {self.F.shape}")
#
        ## Check F top-right block (nu_sf2 for group 1 rows, group 2 columns)
        #print(f"\nF[0,N]   (should be nu_sf2 of first node):  {self.F[0, N]:.6f}")
        #print(f"F[N,0]   (should be 0):                      {self.F[N, 0]:.6f}")
        #print(f"F[0,0]   (should be nu_sf1 of first node):   {self.F[0, 0]:.6f}")
#
        ## Check L scattering block
        #print(f"\nL[N,0]   (should be -sigma_s12 first node): {self.L[N, 0]:.6f}")
        #print(f"L[0,N]   (should be 0):                      {self.L[0, N]:.6f}")
#
        ## Find the mid-fuel node index (should be around node 100)
        #mid = N // 2
        #print(f"\nMid-fuel node {mid}:")
        #print(f"  F[{mid},{mid}]    nu_sf1 = {self.F[mid,mid]:.6f}  (expect 0.006)")
        #print(f"  F[{mid},{N+mid}]  nu_sf2 = {self.F[mid,N+mid]:.6f}  (expect 0.135)")
        #print(f"  L[{mid},{mid}]    L1 diag = {self.L[mid,mid]:.6f}")
        #print(f"  L[{N+mid},{N+mid}] L2 diag = {self.L[N+mid,N+mid]:.6f}")
        ...
        for iteration in range(max_iter):
            ...
            phi_new = np.linalg.solve(self.L, fission_source)
            
            # Update k using fission rate ratio BEFORE normalizing
            k_new = np.sum(self.F @ phi_new) / np.sum(fission_source)
...
if __name__ == "__main__":
    ...
    ax1.plot(mesh.x, solver.phi2, color='blue', linewidth=2)
